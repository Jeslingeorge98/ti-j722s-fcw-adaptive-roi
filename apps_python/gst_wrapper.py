...
def get_pre_proc_elements(flow):
    ...
    if flow.model.task_type == "classification":
        ...
        # type), so read param.yaml directly off the model's own directory.
        with open(os.path.join(flow.model.path, "param.yaml"), "r") as f:
            _params = yaml.safe_load(f)
        crop_align = _params["preprocess"].get("crop_align", "center")
        if crop_align == "bottom":
            # UFLDv2 was trained on bottom-cropped inputs: keep the bottom
            # `crop[1]` rows of the resized frame instead of a centered crop.
            top = flow.pre_proc_resize[1] - flow.model.crop[1]
            bottom = 0
        else:
            top = (flow.pre_proc_resize[1] - flow.model.crop[1]) // 2
            bottom = flow.pre_proc_resize[1] - flow.model.crop[1] - top
